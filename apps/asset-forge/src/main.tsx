...
if ('serviceWorker' in navigator) {
  navigator.serviceWorker.getRegistrations().then(registrations => {
    registrations.forEach(registration => {
      console.log('[HyperForge] Unregistering service worker:', registration.scope)
...
console.log('[HyperForge] Initializing...')
console.log('[HyperForge] Environment:', import.meta.env.MODE)
console.log('[HyperForge] Privy App ID:', import.meta.env.VITE_PUBLIC_PRIVY_APP_ID ? '✅ Set' : '❌ Missing')

// Check for required environment variables
if (!import.meta.env.VITE_PUBLIC_PRIVY_APP_ID) {
  console.error('[HyperForge] ❌ CRITICAL: VITE_PUBLIC_PRIVY_APP_ID is not set!')
  console.error('[HyperForge] Available env vars:', Object.keys(import.meta.env))
}
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 flex items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-gray-400">Loading HyperForge...</p>
        </div>
      </div>
    )
  }
