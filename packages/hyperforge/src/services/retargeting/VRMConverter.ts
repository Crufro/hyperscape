  ...
  private normalizeScale(): void {
    ...
    if (armature && armature.parent) {
      ...
      if (Math.abs(armatureScale - 1.0) > 0.001) {
        ...
        // CRITICAL: Scale geometry vertices to compensate for removing parent scale
        // Without this, the mesh will appear 100x smaller than the skeleton
        if (this.skinnedMesh && this.skinnedMesh.geometry) {
          this.skinnedMesh.geometry.scale(
            compensationScale,
            compensationScale,
            compensationScale,
          );
          console.log(
            `   ✅ Scaled geometry by ${compensationScale.toFixed(1)}x`,
          );
        }
        ...
        // Recalculate skeleton inverse bind matrices to match new geometry and bone positions
