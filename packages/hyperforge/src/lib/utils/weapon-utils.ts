 ...
 * Comprehensive weapon/equipment attachment utilities for VRM characters
 */
 ...
 * Supports various avatar formats: VRM, Mixamo, ReadyPlayerMe, custom
 */
...
/**
 * Get default offset for a weapon type
 */
export function getWeaponOffset(weaponType: string): {
  position: { x: number; y: number; z: number };
  rotation: { x: number; y: number; z: number };
} {
  return WEAPON_OFFSETS[weaponType] || WEAPON_OFFSETS.default;
}
